...
class DS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_ids = self.tokenizer.apply_chat_template(
            msgs_resp, add_generation_prompt=False, truncation=True,
            max_length=self.max_len, padding='max_length', return_tensors=None
        )
        prompt_ids = self.tokenizer.apply_chat_template(
            msgs_prompt, add_generation_prompt=True, truncation=True,
...
# Test: forward only (no optimizer)
...
# Test: forward + backward with no optimizer step
print("\n=== Forward+backward test (no optimizer step) ===")
model.train()
...
for i, batch in enumerate(loader):
    ...
    with torch.autocast('cuda', dtype=torch.float16):
        ...
        loss = out.loss
    loss.backward()
    print(f'  Step {i+1}: loss={loss.item():.4f}, nan={int(torch.isnan(loss).sum().item())}, grad_norm={sum(p.grad.norm().item() for p in model.parameters() if p.grad is not None and not torch.isnan(p.grad).any()):.2f}')
    opt.zero_grad()
    del out, loss

# Test: full train loop
print("\n=== Full train loop (10 steps) ===")
model.train()
opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=2e-4)
for i, batch in enumerate(loader):
    if i >= 10:

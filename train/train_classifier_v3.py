"""
v3: 从 best_classifier.pt 热启动，LR=5e-5（更保守），5 epochs。
目标：在保住 other_diag 的前提下提升主诊断/主手术准确率。
"""
import json
import torch
...
CHECKPOINT_OUT = "/root/autodl-tmp/classifier_output/best_classifier_v3.pt"
...
def eval_model(model, loader, main_diag_classes, main_surg_classes,
    ...
    best_m, best_params = 0, (0.25, 0.4, 3)
...
def main():
    print("加载 checkpoint (best_classifier.pt)...")
    ...
    print("  已加载 best_classifier.pt 权重")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if m > best_m:
            ...
            print(f"  => 保存 best_classifier_v3.pt (M={best_m:.4f})")

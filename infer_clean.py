#!/usr/bin/env python3
"""
InternLM2-1_8B + LoRA 推理脚本 (严格版)
"""
import os
os.environ['TOKENIZERS_PARALLELISM'] = 'false'
os.environ['PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION'] = 'python'
import json, torch
import pandas as pd
from transformers import AutoTokenizer, AutoModelForCausalLM
from peft import PeftModel
from tqdm import tqdm
import re

BASE_MODEL = "/root/autodl-tmp/models/internlm_InternLM2-1_8B"
LORA_PATH = "/root/autodl-tmp/output/icd_lora_1b8/best"
TEST_FILE = "/root/autodl-tmp/A_test.xlsx"
OUTPUT_FILE = "/root/autodl-tmp/predictions_clean.csv"

SYSTEM = "你是一个专业的医学编码助手。根据电子病历文本，预测ICD诊断编码和手术编码。严格按以下格式输出：主要诊断编码|其他诊断编码1;其他诊断编码2;...|主要手术编码|其他手术编码1;其他手术编码2;...某个字段无编码时留空。"
PROMPT = "病历文本：\n{text}\n\n请严格按格式预测ICD编码（只输出编码）："
MAX_TEXT_CHARS = 700


def concat_text(row):
    parts = []
    for col_idx in range(1, 15):
        val = row.iloc[col_idx]
        if pd.notna(val) and str(val).strip():
            parts.append(str(val).strip())
    return '\n'.join(parts)


def is_valid_icd(code):
    """检查是否像有效ICD编码"""
    if not code or len(code) < 3:
        return False
    code = code.strip()
    # 去掉 |im_end 等残留
    code = re.sub(r'[<\[][^\s]*$', '', code).strip()
    if not code or len(code) < 3:
        return False
    # ICD-10-CM: 字母开头，如 A00, C50.900x011, Z98.800x612
    # ICD-9-CM: 数字开头，如 85.4301, 74.1x01
    # 至少要有字母+数字 或 数字+.+数字 的结构
    return bool(re.match(r'^([A-Z][0-9]|[0-9][0-9])', code))


def parse_output(output_text):
    """严格解析模型输出，只提取前4个字段"""
    # 移除特殊token残留
    output_text = output_text.replace('<|im_end|>', '').replace('<|endoftext|>', '').replace('|im_end|', '').strip()

    # 只取第一行（如果有换行）
    first_line = output_text.split('\n')[0].strip()

    # 找第一个 | 作为分隔
    # 但如果模型说了话再出格式，需要提取格式部分
    # 策略：找最后一个 |im_end| 之前的内容，或者直接找格式

    # 提取第一个完整的4字段格式
    # 格式: XXX|XXX|XXX|XXX
    # 先按 | 分割
    parts = first_line.split('|')
    if len(parts) >= 4:
        # 取前4部分
        main_d = parts[0].strip()
        other_d = parts[1].strip()
        main_s = parts[2].strip()
        other_s = parts[3].strip()
    elif len(parts) == 3:
        main_d = parts[0].strip()
        other_d = parts[1].strip()
        main_s = parts[2].strip()
        other_s = ''
    elif len(parts) == 2:
        main_d = parts[0].strip()
        other_d = parts[1].strip()
        main_s = ''
        other_s = ''
    elif len(parts) == 1:
        # 整行作为主诊断
        main_d = parts[0].strip()
        other_d = ''
        main_s = ''
        other_s = ''
    else:
        main_d = other_d = main_s = other_s = ''

    # 清理各字段：只保留有效的ICD编码 token
    def clean_codes_field(s):
        if not s:
            return ''
        # 移除所有特殊token残留
        s = re.sub(r'\|?im_end[|>]*', '', s)
        s = re.sub(r'\|?endoftext', '', s)
        s = re.sub(r'<\|[^|]+\|>', '', s)
        s = re.sub(r'[<\[][^\s]*$', '', s)
        # 只取 | 分割后的第一部分
        if '|' in s:
            s = s.split('|')[0]
        s = s.strip()
        # 纯中文/描述性内容置空
        chinese_chars = sum(1 for c in s if '一' <= c <= '鿿')
        if chinese_chars > len(s) * 0.3:
            return ''
        return s

    # 进一步验证每个编码
    def filter_codes(codes_str):
        if not codes_str:
            return ''
        codes = codes_str.replace('|', ';').split(';')
        valid = []
        for c in codes:
            c = c.strip()
            if is_valid_icd(c):
                valid.append(c)
        return ';'.join(valid)

    main_d = filter_codes(clean_codes_field(main_d))
    other_d = filter_codes(clean_codes_field(other_d))
    main_s = filter_codes(clean_codes_field(main_s))
    other_s = filter_codes(clean_codes_field(other_s))

    # 如果主诊断为空，置空字符串
    if not main_d:
        main_d = ''

    return main_d, other_d, main_s, other_s


def main():
    print("加载模型...")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL, trust_remote_code=True, local_files_only=True)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    print("加载基础模型...")
    base_model = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL, device_map="auto", trust_remote_code=True,
        local_files_only=True, torch_dtype=torch.float16,
    )

    print("加载LoRA权重...")
    model = PeftModel.from_pretrained(base_model, LORA_PATH, device_map="auto")
    model.eval()

    # 读取测试数据
    print("读取测试数据...")
    df = pd.read_excel(TEST_FILE, engine='openpyxl')
    print(f"样本数: {len(df)}")

    texts = []
    ids = []
    for idx, row in df.iterrows():
        texts.append(concat_text(row))
        ids.append(str(row.iloc[0]) if pd.notna(row.iloc[0]) else str(idx))

    # 预测
    results = []
    for i, (id_, text) in enumerate(tqdm(list(zip(ids, texts)), desc="推理")):
        messages = [
            {"role": "system", "content": SYSTEM},
            {"role": "user", "content": PROMPT.format(text=text[:MAX_TEXT_CHARS])},
        ]
        input_text = tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)
        inputs = tokenizer(input_text, return_tensors="pt", truncation=True, max_length=1536)
        inputs = {k: v.cuda() for k, v in inputs.items()}

        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_new_tokens=128,  # 足够生成4个字段
                do_sample=False,
                repetition_penalty=1.1,
                eos_token_id=tokenizer.eos_token_id,
                pad_token_id=tokenizer.pad_token_id,
            )

        output_ids = outputs[0][len(inputs["input_ids"][0]):]
        output_text = tokenizer.decode(output_ids, skip_special_tokens=True).strip()

        main_d, other_d, main_s, other_s = parse_output(output_text)

        results.append({
            "病案标识": id_,
            "主要诊断编码": main_d,
            "其他诊断编码": other_d,
            "主要手术编码": main_s,
            "其他手术编码": other_s,
        })

        if i < 5:
            print(f"\n[{i+1}] ID={id_}")
            print(f"  原文: {output_text[:80]}")
            print(f"  解析: {main_d}|{other_d[:40] if other_d else ''}|{main_s}|{other_s[:30] if other_s else ''}")

    # 保存
    result_df = pd.DataFrame(results)
    result_df.to_csv(OUTPUT_FILE, index=False, encoding="utf-8-sig")
    print(f"\n结果已保存: {OUTPUT_FILE}")


if __name__ == "__main__":
    main()

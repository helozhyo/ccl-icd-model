#!/usr/bin/env python3
"""
CCL 2026 ICD - LoRA微调 (InternLM2-1_8B, 全量GPU)
"""
...
class ICDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # labels: prompt部分=-100, response部分=token_id
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        print(f"   GPU mem after load: {mem:.2f}GB, peak: {peak:.2f}GB")
    ...
    dev_loader = DataLoader(
        dev_ds, batch_size=CFG["batch_size"], collate_fn=collate_fn,
    )
    ...
    num_update_steps_per_epoch = len(train_loader) // CFG["gradient_accumulation"]
    ...
    print(f"\n总步数: {num_training_steps}, 预热: {num_warmup_steps}, "
          f"更新次数/epoch: {num_update_steps_per_epoch}")
    print(f"batch_size={CFG['batch_size']}, grad_accum={CFG['gradient_accumulation']}, "
          f"effective_batch={CFG['batch_size']*CFG['gradient_accumulation']}")
